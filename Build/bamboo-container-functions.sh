...
function runLint() {
    docker run \
        ...
        bin/bash -c "cd ${PWD}; find . -path ./vendor -prune -o -path ./typo3 -prune -o -path ./typo3temp -prune -o -name \*.php -print0 | xargs -0 -n1 -P2 php -n -c /etc/php/cli-no-xdebug/php.ini -d display_errors=stderr -l >/dev/null"
}
